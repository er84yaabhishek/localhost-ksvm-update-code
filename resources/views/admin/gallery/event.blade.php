                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <div class="row">
                                    <div class="col-lg-4">
                                        <div class="card-title d-inline-block">Event</div>
                                    </div>
                                                    ...
                                                    @if($event->count() > 0)
                                                        @foreach($event as $item)
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <img src="{{ public_asset('storage/event/' . $item->image) }}" alt=""
                                                                        width="100">
                                                                </td>
                                                                <td>{{ $item->title }}</td>
                                                                <td>
                                                                    <span
                                                                        class="badge badge-{{ $item->status == 1 ? 'success' : 'danger' }}">{{ $item->status == 1 ? 'Active' : 'Inactive' }}</span>
                                                                </td>
                                                                <td>
                                                                    ...
                                                                    <form class="d-inline-block"
                                                                        action="{{ route('admin.event.delete') }}"
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        hideConfirmModal();
                        bootnotify('Event Delete Successfully!', 'Event Delete!', 'success');
                        loadWindow();
                    }
                },
                error: function (xhr, status, error) {
                    bootnotify('Falied Event Delete', 'Event Delete!', 'danger')
                }
            });
        }
    </script>
